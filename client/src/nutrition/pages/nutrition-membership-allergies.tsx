...
import { ArrowLeft, Check, PencilLine, ShieldAlert } from "lucide-react";
...
const PROFILE_SETUP_STEP = MEMBERSHIP_STEPS.allergies;
const PROFILE_SETUP_TOTAL_STEPS = MEMBERSHIP_TOTAL_STEPS;

export default function NutritionMembershipAllergiesPage() {
  ...
  const [foodAllergies, setFoodAllergies] = useState(formState.foodAllergies ?? "");
  const shouldPersistEdit = isReturningToProfileHomeReview(searchParams);
  const backHref = resolveProfileHomeReviewAwareHref("/nutrition/membership/medications-and-supplements", searchParams);
  const nextHref = resolveProfileHomeReviewAwareHref("/nutrition/membership/disliked-foods", searchParams);
  ...
  useEffect(() => {
    ...
    if (!shouldPersistEdit && formState.medicationsAndSupplements === undefined) {
      setLocation("/nutrition/membership/medications-and-supplements");
    }
  }, [formState.medicationsAndSupplements, formState.targetWeightKg, isLoading, setLocation, shouldPersistEdit, user]);

  const handleContinue = async (value = foodAllergies) => {
    const latestFormState = getNutritionFormState();
    const normalizedAllergies = value.trim();
    updateNutritionFormState({
      ...
      foodAllergies: normalizedAllergies,
    });
    if (shouldPersistEdit) {
      const result = await saveMembershipProfileEdit({ step: "allergies", foodAllergies: normalizedAllergies });
      if (!result.success) {
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-[#0a1224] text-white" dir={dir}>
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[390px] flex-col px-5 pb-8 pt-7">
        <NutritionTopbar backHref={backHref} title={t("nutritionMembershipShared.topbarTitle")} description={t("nutritionMembershipAllergies.topbarDescription")} variant="hero" />
        ...
        <main className="mt-7 rounded-[28px] border border-white/10 bg-[#070d18]/35 px-5 pb-5 pt-8 shadow-[0_30px_85px_-55px_rgba(0,0,0,0.95)]">
          <div className="mx-auto flex h-[64px] w-[64px] items-center justify-center rounded-[22px] border border-rose-300/24 bg-rose-400/10 text-rose-300 shadow-[0_24px_55px_-38px_rgba(251,113,133,0.75)]">
            <ShieldAlert className="h-8 w-8" />
          </div>

          <div className="mt-6 space-y-3 text-center">
            <h1 className="text-[26px] font-black leading-[1.55] text-white">{t("nutritionMembershipAllergies.title")}</h1>
            <p className="mx-auto max-w-[300px] text-[13px] font-bold leading-7 text-slate-400">
              {t("nutritionMembershipAllergies.description")}
            </p>
          </div>
          ...
          <div className="mt-7 rounded-[24px] border border-amber-300/28 bg-[#070c14]/70 p-4 shadow-[0_20px_50px_-38px_rgba(251,191,36,0.75)]">
            <div className="mb-3 flex items-center justify-between gap-3">
              <label htmlFor="food-allergies" className="flex items-center gap-2 text-[14px] font-black text-amber-300">
            ...
            >
              ...
              {t("nutritionMembershipAllergies.none")}
            </Button>
          </div>
        </main>
