...
export default function PellehFeatureDetailPage() {
  ...
  return <div dir="rtl" lang="fa" className="min-h-screen bg-[#0e0d0b] text-[#f4f2ee]">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0e0d0bd9] backdrop-blur-xl"><div className="mx-auto flex max-w-[1200px] items-center justify-between px-8 py-5"><a href="/" className="flex items-center gap-2.5 text-white"><span className="flex size-8 items-center justify-center rounded-full border border-[#c9a24a] text-[#e0c06e]">پ</span><strong>پلـه</strong></a><a href="/plans" className="rounded-full border border-[#c9a24a] px-5 py-2.5 text-sm text-[#e0c06e]">شروع خرید پکیج</a></div></header>
    <main>
      ...
      <section className="mx-auto grid max-w-[1000px] grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6 px-5 pt-12"><div className="min-h-60 overflow-hidden rounded-[20px] border border-white/10"><img src={item.images[0] || defaultHeroImage} alt={item.title} className="h-full min-h-60 w-full object-cover"/></div><div className="flex flex-col justify-center">{(item.benefits.length ? item.benefits : [item.short, "مدیریت ساده و یکپارچه از پنل پله"]).filter(Boolean).map((benefit, benefitIndex)=><div key={benefitIndex} className="flex gap-3 border-b border-white/10 py-3.5 text-sm leading-7"><span className="text-[#e0c06e]">✓</span><span>{benefit}</span></div>)}</div></section>
      ...
      <section className="mx-auto mt-16 max-w-[1200px] border-t border-white/10 px-5 py-20 text-center"><h2 className="mb-5 text-xl font-extrabold">یک پله بالاتر باشید</h2><a href="/plans" className="inline-block rounded-full bg-[#c9a24a] px-7 py-3.5 text-sm font-bold text-[#0e0d0b]">شروع خرید پکیج</a></section>
    </main>
  </div>;
}
