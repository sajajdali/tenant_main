...
در تنظیمات پرداخت، «درگاه پرداخت بازار برای اپلیکیشن» را فعال کنید و این سه مقدار را از پنل توسعه‌دهندگان Bazaar ثبت کنید:

- `packageName`: نام package واقعی APK، مانند `ir.example.nutrition`
- `clientId`
- `clientSecret`

`clientSecret` فقط روی سرور ذخیره می‌شود و API تنظیمات آن را برنمی‌گرداند. برای هر پکیج تغذیه فعال نیز مقدار `cafebazaarProductId` را برابر Product ID محصول **مصرفی** همان پکیج در پنل Bazaar قرار دهید. قیمت خرید، قیمت تعریف‌شده در Bazaar است؛ قیمت وب را برای خرید Bazaar نمایش/مبنا قرار ندهید.

## امنیت و قانون قطعی

Flutter فقط خرید را با SDK/کتابخانه Bazaar انجام می‌دهد. اعتبارسنجی و مصرف فقط در backend انجام می‌شود. Flutter نباید `clientSecret`، access token، درخواست مستقیم به `pardakht.cafebazaar.ir`، `consumeProduct`، `signed_data` یا `signature` را ارسال یا نگهداری کند.
...
3. سرور از Bazaar access token می‌گیرد و `validate` را فراخوانی می‌کند.
4. فقط `purchaseState = 0` و `consumptionState = 0` موفق تلقی می‌شود.
...
پس از consume موفق، پاسخ Bazaar معمولاً `consumptionState: 1` را نشان می‌دهد. Flutter فقط موفقیت HTTP 200 endpoint خودمان را معیار نمایش موفقیت قرار دهد؛ consume دیگری انجام ندهد.
