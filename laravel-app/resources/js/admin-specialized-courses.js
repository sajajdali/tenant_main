...
const syncHiddenFromDisplay = (displayInput, hiddenInput, boundary) => {
    ...
    hiddenInput.value = `${gy}-${pad(gm)}-${pad(gd)} ${boundary === 'end' ? '23:59:59' : '00:00:00'}`;
};
...
const initSelect2 = (selector, options = {}) => {
    ...
    if (!element || typeof window.jQuery?.fn?.select2 !== 'function') {
        return;
    }
...
const categoryMatcher = (params, data) => {
    const audienceField = document.getElementById('audience_type_id');
    const selectedAudienceId = String(audienceField?.value || '');
    const optionAudienceId = String(data.element?.dataset?.audienceId || '');

    if (data.id && selectedAudienceId && optionAudienceId !== selectedAudienceId) {
        return null;
    }
    ...
    return String(data.text || '').includes(String(params.term).trim()) ? data : null;
};

const syncCategoryAudience = () => {
    const audienceField = document.getElementById('audience_type_id');
    const categoryField = document.getElementById('specialized_course_category_id');
    const categoryHint = document.getElementById('category_audience_hint');

    if (!audienceField || !categoryField) {
        return;
    }
    ...
    Array.from(categoryField.options).forEach((option) => {
        const optionAudienceId = String(option.dataset.audienceId || '');
        const isPlaceholder = option.value === '';
        const isVisible = isPlaceholder || !selectedAudienceId || optionAudienceId === selectedAudienceId;
    ...
    if (!hasVisibleSelectedOption && categoryField.value) {
        categoryField.value = '';
        window.jQuery?.(categoryField).trigger('change.select2');
    }

    if (categoryHint) {
        categoryHint.textContent = selectedAudienceId
            ? 'فقط گروه‌بندی‌های مربوط به همین طیف نمایش داده می‌شوند.'
            : 'ابتدا طیف را انتخاب کنید تا گروه‌بندی‌های همان طیف نمایش داده شوند.';
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    connectDateInput({
        displayId: 'discount_ends_at_display',
        hiddenId: 'discount_ends_at',
        boundary: 'end',
    });

    initSelect2('#teacher_user_id', {
        placeholder: 'مدرس را انتخاب کنید',
    });

    initSelect2('#audience_type_id', {
        placeholder: 'طیف را انتخاب کنید',
        minimumResultsForSearch: Infinity,
    });

    initSelect2('#specialized_course_category_id', {
        placeholder: 'گروه‌بندی دوره را انتخاب کنید',
        matcher: categoryMatcher,
    });

    syncCategoryAudience();
    document.getElementById('audience_type_id')?.addEventListener('change', () => {
        syncCategoryAudience();
        window.jQuery?.('#specialized_course_category_id').trigger('change.select2');
    });
});
